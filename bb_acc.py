...
def get_wallet_balance(session: HTTP):
    ...
    Args:
        session: The HTTP session to use for API calls
    """
    logging.info("Fetching wallet balance data...")
    res = session.get_wallet_balance(accountType="UNIFIED")

    if not res["retCode"] == 0:
        msg = res.get("retMsg", "Unknown error")
        logging.error(f"Error getting wallet balance: {msg}")
        raise Exception(f"Error getting wallet balance: {msg}")
    
    result = res.get("result", {})
    if not result:
        logging.warning("No result data in wallet balance response")
        return

    account_info = result.get("list", [{}])[0]

    if not account_info:
        logging.warning("No account info in wallet balance response")
        return
...
def get_coin_greeks(session: HTTP, base_coin: str | None = None):
    ...
    Args:
        session: The HTTP session to use for API calls
        base_coins: List of coins to get greeks for
    """
    logging.info(f"Fetching coin greeks data for base_coin: {base_coin or 'all'}")

    if base_coin is None:
        res = session.get_coin_greeks()
    else:
        res = session.get_coin_greeks(baseCoin=base_coin)

    if not res["retCode"] == 0:
        msg = res.get("retMsg", "Unknown error")
        logging.error(f"Error getting coin greeks: {msg}")
        raise Exception(f"Error getting coin greeks: {msg}")
    
    result = res.get("result", {})
    if not result:
        logging.warning("No result data in coin greeks response")
        return
...
def main():
    ...
    scheduled_data_collection() # Initial run before scheduling
